...
const BkashPayModal = ({allTotal}) => {
  ...
  const [bkash, setBkash] = useState(initialState);
  const handleChangeInput = (e) => {
    const { value, name } = e.target;
    setBkash({ ...bkash, [name]: value });
  };
    ...
    <div>
      {!no ? (
        <form onSubmit={handleSubmit}>
          <div className="mb-3">
            <label className="form-label">Bkash No</label>
            <input
              type="text"
              name="no"
              className="form-control"
              style={{ backgroundColor: "#CA1F50", color: "#fff" }}
        ...
        <form onSubmit={handleOrder}>
          <div className="mb-3">
            <label className="form-label">Bkash OTP</label>
            <input
              type="text"
              name="otp"
              className="form-control"
